...
const FindJob = ({route}) => {
    ...
    const specialize = route.params.specialize;
    const jobPosition = route.params.jobPosition;
    const typeWork = route.params.typeWork;
    const typeJob = route.params.typeJob;
    const jobLocation = route.params.jobLocation;
    const minWage = route.params.minWage;
    const maxWage = route.params.maxWage;
        ...
        <Container>
            <SafeAreaView style={styles.container}>
                <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
                    ...
                    <Text style={styles.textFindJob}>Danh sách công việc</Text>
                    <Text></Text>
                </View>
                <Text style={{ color: '#AAA6B9', fontSize: 14, fontStyle: 'italic', marginBottom: 10}}>"Đang tìm kiếm theo chuyên môn: {specialize}, vị trí công việc: {jobPosition}, thời gian làm việc: {typeJob}, nơi làm việc: {jobLocation}, kiểu làm việc: {typeWork}, mức lương trong khoảng từ {minWage} đến {maxWage}"</Text>
